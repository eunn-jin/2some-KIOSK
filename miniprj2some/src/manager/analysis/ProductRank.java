...
public class ProductRank {	// 누적 상품 판매 순위
	...
	public void getDayRank() {
		...
		String sql = "SELECT MN_NAME, NUM "
				...
				+ "FROM ORDER_ITEM "
				+ "WHERE ORDER_ITEM.GROUP_NO IN (SELECT GROUP_NO FROM ORDER_GROUP WHERE TO_CHAR(ORDER_DATE, 'YY/MM/DD') = TO_CHAR(?, 'YY/MM/DD')) "
	...
	public void getPeriodRank() {
		...
		String sql = "SELECT MN_NAME, NUM "
				+ "FROM (SELECT MN_IDX, SUM(ITEM_NUM) NUM "
				+ "FROM ORDER_ITEM "
				+ "WHERE ORDER_ITEM.GROUP_NO IN (SELECT GROUP_NO FROM ORDER_GROUP WHERE TO_CHAR(ORDER_DATE, 'YY/MM/DD') BETWEEN TO_CHAR(?, 'YY/MM/DD') AND TO_CHAR(?, 'YY/MM/DD')) "
				+ "GROUP BY MN_IDX) T JOIN MENU "
				+ "ON T.MN_IDX = MENU.MN_IDX "
				+ "ORDER BY NUM DESC";
		PreparedStatement pstmt = null;
